...
assert.match(mobileUi, /\['capacitor:','ionic:'\]\.includes\(location\.protocol\)\?2\.2:1/)
assert.match(mobileUi, /rawSum\+=raw\*raw/)
assert.match(mobileUi, /Math\.tanh\(raw\*inputGain\)/)
assert.match(mobileUi, /duringSpeech&&isNativeMobileRuntime\(\)/)
assert.match(mobileUi, /async function playableAudioBlob/)
assert.match(mobileUi, /audio\/wav/)
assert.match(mobileUi, /playbackRetry\+\+<1/)
...
assert.match(appDelegate, /\.allowBluetoothA2DP/)
assert.match(appDelegate, /overrideOutputAudioPort\(\.speaker\)/)
assert.doesNotMatch(appDelegate, /\[weak self\] _ in\s*\n\s*self\?\.configureCallAudio\(\)/)
